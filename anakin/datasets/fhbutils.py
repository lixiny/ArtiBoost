...
def load_manofits(sample_infos, fit_root="assets/hasson20_assets/fhbhands_fits"):
    obj_paths = []
    metas = []
...
def load_object_infos(seq_root="data/fhbhands/Object_6D_pose_annotation_v1_1"):
    subjects = sorted(os.listdir(seq_root))
    annots = {}
    clip_lengths = {}
    for subject in subjects:
        ...
        for action in actions:
            ...
            seqs = sorted(os.listdir(action_path))
            clips += len(seqs)
            for seq in seqs:
...
def get_skeletons(skeleton_root, subjects_info, use_cache=True):
    ...
    if os.path.exists(cache_path) and use_cache:
        ...
        logger.info("Loaded fhb skel info from {}".format(cache_path))
    else:
        skelet_dict = defaultdict(dict)
        for subject, samples in etqdm(subjects_info.items(), desc="subj"):
            for (action, seq_idx) in etqdm(samples, desc="sample"):
